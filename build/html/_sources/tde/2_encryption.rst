...
OSコマンドでUSERS表領域のデータファイルの中身を確認します。SQL*Plusでは ``!`` を先頭に使用するとOSコマンドを実行できます。
...
このようにTDEで暗号化されていない場合、 ``strings`` コマンドでデータの内容が確認できてしまうことが分かります。
...
では、HRスキーマが格納されるUSERS表領域をオープンにしたまま暗号化してみます。
